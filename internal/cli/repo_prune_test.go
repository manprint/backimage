...
func TestPrunePlanText(t *testing.T) {
	...
	empty := prunePlanText(false, registry.Policy{}, nil, nil)
	...
	dry := prunePlanText(true, p, []registry.TagInfo{tag("keep", created)}, []registry.TagInfo{tag("old", created)})
	...
	applied := prunePlanText(false, p, nil, []registry.TagInfo{tag("old", time.Time{})})
	...
	none := prunePlanText(false, p, []registry.TagInfo{tag("a", created)}, nil)
